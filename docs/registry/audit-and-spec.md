...
Release events use small reusable-workflow callers and a write deploy key scoped only to `kujolang/kennel-registry`, stored as Actions secrets only in Kennel and Changebucket. This avoids personal tokens and account-wide credentials. The registry's own reconciliation/backfill uses its repository GITHUB_TOKEN; scheduled reconciliation also recovers missed events. Concurrent cross-repository push races fail closed and are recovered by reconciliation.
