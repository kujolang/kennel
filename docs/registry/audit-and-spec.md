...
* Release packaging is a central Kujo-native build tool operating on Git objects at an exact release commit; no working-tree files or package hooks execute. Normalize gzip mtime, USTAR timestamps, uid/gid, modes, names and order. Reject links/submodules, unsafe names and collisions. Neither publishing nor client installation requires Python. Independent adversarial archive tests use Python only as a test harness.
